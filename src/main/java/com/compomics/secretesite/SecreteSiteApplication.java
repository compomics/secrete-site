...
public class SecreteSiteApplication {
    ...
    public SecreteSiteApplication(SpeciesRepository speciesRepository, GeneRepository geneRepository) {
            ...
            try (LineNumberReader reader = new LineNumberReader(new FileReader(new File(args[0], "Sc_resultstable_enriched.txt")))) {
                ...
                while (line != null) {
                    ...
                    List<Transcript> transcripts = gene.getTranscripts()
                            .stream()
                            .filter(e -> e.getEnsembleTranscriptId().equals(transcript.getEnsembleTranscriptId()) && e.getTranscript_sequence().equals(transcript.getTranscript_sequence()))
